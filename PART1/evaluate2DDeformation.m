function [BminusA, Npeaks, Nvalleys] = evaluate2DDeformation(idx1,idx2)
%clear all;
close all;
...
I = res_norm1;
imwrite(I,['.\interferencePattern\ip_' ,idex{1},'-',idex{2}, '.png' ]); 
...
Nvalleys = (mean(max(res_valley')));
disp(['',num2str(Nvalleys,2),' valleys']);
BminusA = (Npeaks+Nvalleys)*543/4;
disp(['(b-a): ',num2str(BminusA,4), ' nm']);
disp(['angle: ',num2str(BminusA/(6.7*10^6),2), ' rad, or ',...
    num2str(180/pi*BminusA/(6.7*10^6),2), 'º']);
